...
estimated from these data: r̂² and β̂₁ / β̂\*₂\*\*

Use 5000 bootstrap samples and, for each bootstrap sample, produce
estimates of these two quantities. Plot the distribution of your
estimates, and describe these in words. Using the 5000 bootstrap
estimates, identify the 2.5% and 97.5% quantiles to provide a 95%
confidence interval for r̂² and β̂₁ / β̂₂.

Note: broom::glance() is helpful for extracting r̂² from a fitted
regression, and broom::tidy() (with some additional wrangling) should
help in computing β̂₁ / β̂₂.
...
boot_results = 
  ...
  mutate(
    ...
    # SLR with response: tmax, predictors: tmin, prcp
  ...
  select(-strap, -models) |> 
  # unnest to get the R-squared values
  unnest(results_glance) |> 
  select(.id, r.squared, results_tidy) |> 
  # unnest to get the coefficients
  unnest(results_tidy) |> 
  select(.id, r.squared, term, estimate) |> 
  
  # reshape so tmin and prcp estimates are in their own columns
  pivot_wider(
    names_from = term,
    values_from = estimate) |> 
  
  # Calculate the ratio: beta_1 (tmin) / beta_2 (prcp)
...
```
    ...
    ## # A tibble: 6 × 6
    ##   .id   r.squared `(Intercept)`  tmin      prcp beta_ratio
    ##   <chr>     <dbl>         <dbl> <dbl>     <dbl>      <dbl>
    ## 1 0001      0.917          7.38  1.05 -0.00200       -527.
    ## 2 0002      0.915          7.58  1.04 -0.00146       -710.
    ## 3 0003      0.916          7.43  1.04 -0.000867     -1195.
    ## 4 0004      0.910          7.64  1.03 -0.00130       -787.
    ## 5 0005      0.907          7.47  1.03  0.000182      5670.
    ## 6 0006      0.903          7.31  1.03  0.00186        552.

``` r
# 1. Generate 5000 bootstrap samples and fit models
boot_results_2 = 
  central_park_df |> 
  modelr::bootstrap(n = 5000) |> 
  mutate(
  ...
  # Calculate the ratio (tmin / prcp)
  mutate(
    beta_ratio = tmin / prcp
  )

# 2. Create a "Better Table" (Confidence Intervals)
ci_table = 
  boot_results |> 
  summarize(
    ci_lower_r2 = quantile(r.squared, 0.025),
    ci_upper_r2 = quantile(r.squared, 0.975),
    ci_lower_ratio = quantile(beta_ratio, 0.025),
    ci_upper_ratio = quantile(beta_ratio, 0.975)
  ) |> 
  # Reshape to make the table easier to read
  pivot_longer(
    everything(),
    names_to = "measure", 
    values_to = "value"
  ) |> 
  # Separate the measure name from the limit type (lower/upper)
...
ci_table |> 
  knitr::kable(digits = 3, caption = "95% Confidence Intervals from 5000 Bootstrap Samples", align = "c")
```

|         Metric         | 2.5% Quantile | 97.5% Quantile |
|:----------------------:|:-------------:|:--------------:|
|       R-squared        |     0.894     |     0.928      |
| Beta Ratio (tmin/prcp) |   -5344.442   |    4320.356    |
...
![](p8105_hw6_lg3450_files/figure-gfm/unnamed-chunk-10-1.png)<!-- -->
